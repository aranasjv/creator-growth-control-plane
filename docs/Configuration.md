...
- `google_maps_scraper_niche`: `string` - The niche you want to scrape Google Maps for. This can now be a single niche or a comma-separated list such as `pet shops, dental clinics, local clinics`.
- `scraper_timeout`: `number` - The timeout for the Google Maps scraper.
- `scraper_depth`: `number` - Search-result scroll depth passed to the Google Maps scraper. Lower values are safer for desktop dry runs (default: `1`).
- `scraper_concurrency`: `number` - Scraper concurrency. Lower values reduce browser instability on local machines (default: `1`).
- `scraper_exit_on_inactivity`: `string` - Inactivity duration string passed through to the scraper (default: `90s`).
- `outreach_max_emails_per_run`: `number` - Maximum number of live emails to send in one outreach run (default: `10`).
...
- `CGCP_OUTREACH_DRY_RUN`: when set to `1`, `true`, or `yes`, outreach will scrape and prepare email targets without sending live email.
- `CGCP_OUTREACH_NICHES`: overrides `google_maps_scraper_niche` for a run and supports comma-separated niche values.
- `CGCP_OUTREACH_TIMEOUT`: overrides `scraper_timeout` for a run.
- `CGCP_OUTREACH_DEPTH`: overrides `scraper_depth` for a run.
- `CGCP_OUTREACH_CONCURRENCY`: overrides `scraper_concurrency` for a run.
- `CGCP_OUTREACH_EXIT_ON_INACTIVITY`: overrides `scraper_exit_on_inactivity` for a run.
- `CGCP_OUTREACH_MAX_EMAILS`: overrides `outreach_max_emails_per_run` for a run.
