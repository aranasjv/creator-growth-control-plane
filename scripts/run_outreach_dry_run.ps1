...
if (-not $existingWorker) {
  ...
  Write-Host "[outreach] Reusing existing worker process $($existingWorker.ProcessId)"
}
...
try {
  ...
  $job = Invoke-RestMethod -Method Post -Uri 'http://localhost:5050/api/jobs' -ContentType 'application/json' -Body (@{
    ...
    provider = 'outreach'
    parameters = @{
      source = 'scripts/run_outreach_dry_run.ps1'
      mode = 'dry-run'
      niche = $Niches
      niches = $Niches
  ...
  $jobId = $job.jobId
  Write-Host "[outreach] Queued dry-run job $jobId for niches: $Niches"
  ...
  for ($attempt = 0; $attempt -lt $maxAttempts; $attempt++) {
    ...
    if ($jobState.status -in @('succeeded', 'failed', 'cancelled')) {
      Write-Host "[outreach] Final status: $($jobState.status)"
      if ($jobState.resultJson) {
        Write-Host "[outreach] Result: $($jobState.resultJson)"
      }

      if ($jobState.status -ne 'succeeded') {
        throw "Outreach dry run failed: $($jobState.errorMessage)"
      }
  ...
  $jobTimedOut = $true
  throw 'Timed out waiting for outreach dry-run completion.'
}
finally {
  if ($startedWorker) {
